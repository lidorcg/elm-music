...
-}
module GraphQL.Playlists exposing (playlists, PlaylistsResult, createPlaylist, CreatePlaylistResult, renamePlaylist, RenamePlaylistResult, deletePlaylist, DeletePlaylistResult, addTrackToPlaylist, AddTrackToPlaylistResult)
...
type alias RenamePlaylistResult 
    = { renamePlaylist :     { ok : (Maybe Bool)
    , playlists : (List         { id : String
        , name : (Maybe String)
        , tracks : (List             { id : String
            , name : (Maybe String)
...
renamePlaylist :     { id : String
    , name : String
    } -> Task Http.Error RenamePlaylistResult
renamePlaylist params =
    let graphQLQuery = """mutation renamePlaylist($id: String!, $name: String!) { renamePlaylist(playlistId: $id, newName: $name) { ok playlists { id name tracks { id name artists duration youtubeId } } } }""" in
    let graphQLParams =
            Json.Encode.object
    ...
    in
    GraphQL.mutation endpointUrl graphQLQuery "renamePlaylist" (encode 0 graphQLParams) renamePlaylistResult


renamePlaylistResult : Decoder RenamePlaylistResult
renamePlaylistResult =
    map RenamePlaylistResult ("renamePlaylist" :=
        (map (\ok playlists -> { ok = ok, playlists = playlists }) (maybe ("ok" := bool))
        `apply` ("playlists" :=
        (list (map (\id name tracks -> { id = id, name = name, tracks = tracks }) ("id" := string)
        `apply` (maybe ("name" := string))
...
type alias DeletePlaylistResult 
    = { deletePlaylist :     { ok : (Maybe Bool)
    , playlists : (List         { id : String
        , name : (Maybe String)
        , tracks : (List             { id : String
            , name : (Maybe String)
...
deletePlaylist :     { id : String
    } -> Task Http.Error DeletePlaylistResult
deletePlaylist params =
    let graphQLQuery = """mutation deletePlaylist($id: String!) { deletePlaylist(playlistId: $id) { ok playlists { id name tracks { id name artists duration youtubeId } } } }""" in
    let graphQLParams =
            Json.Encode.object
                [ ("id", Json.Encode.string params.id)
                ]
    in
    GraphQL.mutation endpointUrl graphQLQuery "deletePlaylist" (encode 0 graphQLParams) deletePlaylistResult


deletePlaylistResult : Decoder DeletePlaylistResult
deletePlaylistResult =
    map DeletePlaylistResult ("deletePlaylist" :=
        (map (\ok playlists -> { ok = ok, playlists = playlists }) (maybe ("ok" := bool))
        `apply` ("playlists" :=
        (list (map (\id name tracks -> { id = id, name = name, tracks = tracks }) ("id" := string)
        `apply` (maybe ("name" := string))
...
type alias AddTrackToPlaylistResult 
    = { addTrack :     { ok : (Maybe Bool)
    , playlists : (List         { id : String
        , name : (Maybe String)
        , tracks : (List             { id : String
            , name : (Maybe String)
...
addTrackToPlaylist :     { track :         { name : String
        , duration : String
        , artists : String
        , youtubeId : (Maybe String)
        }
    , playlistId : String
    } -> Task Http.Error AddTrackToPlaylistResult
addTrackToPlaylist params =
    let graphQLQuery = """mutation addTrackToPlaylist($track: TrackInput!, $playlistId: String!) { addTrack(track: $track, playlistId: $playlistId) { ok playlists { id name tracks { id name artists duration youtubeId } } } }""" in
    let graphQLParams =
            Json.Encode.object
    ...
    in
    GraphQL.mutation endpointUrl graphQLQuery "addTrackToPlaylist" (encode 0 graphQLParams) addTrackToPlaylistResult


addTrackToPlaylistResult : Decoder AddTrackToPlaylistResult
addTrackToPlaylistResult =
    map AddTrackToPlaylistResult ("addTrack" :=
        (map (\ok playlists -> { ok = ok, playlists = playlists }) (maybe ("ok" := bool))
        `apply` ("playlists" :=
        (list (map (\id name tracks -> { id = id, name = name, tracks = tracks }) ("id" := string)
        `apply` (maybe ("name" := string))
